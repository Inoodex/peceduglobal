<template>
  <MainLayout>
    <div class="max-w-7xl mx-auto p-6">
      <h1 class="text-2xl font-bold mb-4">Edit Block</h1>
...
const fetchBlock = async () => {
  try {
    const response = await axios.get(`/api/admin/blocks/${route.params.id}`);
...
const save = async () => {
  try {
    await axios.put(`/api/admin/blocks/${route.params.id}`, form.value);
