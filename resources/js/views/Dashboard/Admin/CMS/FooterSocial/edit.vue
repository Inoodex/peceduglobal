<template>
  <MainLayout>
    ...
    <div class="flex items-center gap-4">
      ...
      <h1 class="text-2xl font-bold text-gray-900 dark:text-white tracking-tight">Edit Social Link</h1>
    </div>
...
const fetchSocial = async () => {
  try {
    ...
    const social = res.data.data.find(s => s.id === parseInt(route.params.id));
    if (social) {
      form.value = {
        name: social.name,
        url: social.url,
        serial_no: social.serial_no,
    ...
    toast.error('Failed to fetch social link data.');
    router.push('/dashboard/footer-socials');
  } finally {
    loading.value = false;
  }
...
const submitForm = async () => {
  saving.value = true;
  try {
    const res = await axios.put(`/auth/admin/footer-socials/${route.params.id}`, form.value);
    if (res.data.success) {
